...
SWITCH_DECLARE(uint8_t) switch_stun_packet_attribute_add_integrity(switch_stun_packet_t *packet, const char *pass)
{
	...
	attribute = (switch_stun_packet_attribute_t *) ((uint8_t *) & packet->first_attribute + ntohs(packet->header.length));
	attribute->type = htons(SWITCH_STUN_ATTR_MESSAGE_INTEGRITY);
	attribute->length = htons(20);
...
SWITCH_DECLARE(uint8_t) switch_stun_packet_attribute_add_fingerprint(switch_stun_packet_t *packet)
{
	...
	attribute = (switch_stun_packet_attribute_t *) ((uint8_t *) & packet->first_attribute + ntohs(packet->header.length));
	attribute->type = htons(SWITCH_STUN_ATTR_FINGERPRINT);
	attribute->length = htons(4);
...
SWITCH_DECLARE(uint8_t) switch_stun_packet_attribute_add_software(switch_stun_packet_t *packet, char *software, uint16_t ulen)
{
	...
	attribute = (switch_stun_packet_attribute_t *) ((uint8_t *) & packet->first_attribute + ntohs(packet->header.length));
	attribute->type = htons(SWITCH_STUN_ATTR_SOFTWARE);
	attribute->length = htons(ulen);
	if (software) {
		memcpy(attribute->value, software, ulen);
	} else {
		switch_stun_random_string(attribute->value, ulen, NULL);
	}
...
SWITCH_DECLARE(switch_status_t) switch_stun_lookup_ipv4v6(int32_t family, char **ip, switch_port_t *port, char *stunip, switch_port_t stunport, char **err, switch_memory_pool_t *pool)
{
	...
	*err = "Success";
	...
	res = switch_sockaddr_info_get(&from_addr, NULL, family, 0, 0, pool);
	(void)res;
...
/*
SWITCH_DECLARE(switch_status_t) switch_stun_lookup_ip_v6(char** ip, switch_port_t* port, char* stunip, switch_port_t stunport, char** err, switch_memory_pool_t* pool)
{
	switch_sockaddr_t *local_addr = NULL, *remote_addr = NULL, *from_addr = NULL;
	switch_socket_t *sock = NULL;
	uint8_t buf[260] = {0};
	...
	if (*err && !strcmp(*err, "funny")) { funny = 1; }

	*err = "Success";

	switch_sockaddr_info_get(&from_addr, NULL, SWITCH_INET6, 0, 0, pool);

	if (switch_sockaddr_info_get(&local_addr, *ip, SWITCH_INET6, *port, 0, pool) != SWITCH_STATUS_SUCCESS) {
		*err = "Local Address Error!";
		return SWITCH_STATUS_FALSE;
	}

	if (switch_sockaddr_info_get(&remote_addr, stunip, SWITCH_INET6, stunport, 0, pool) != SWITCH_STATUS_SUCCESS) {
		*err = "Remote Address Error!";
		return SWITCH_STATUS_FALSE;
	}

	if (switch_socket_create(&sock, AF_INET6, SOCK_DGRAM, 0, pool) != SWITCH_STATUS_SUCCESS) {
		*err = "Socket Error!";
		return SWITCH_STATUS_FALSE;
	}
	...
	*port = 0;
	...
	do {
		...
		if (!switch_stun_packet_next_attribute(attr, end_buf)) { break; }
